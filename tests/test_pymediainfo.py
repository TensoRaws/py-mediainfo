...
class MediaInfoTest(unittest.TestCase):
    ...
    def test_track_other_attributes(self):
        general_tracks = [track for track in self.media_info.tracks if track.track_type == "General"]
        general_track = general_tracks[0]
        self.assertEqual(5, len(general_track.other_file_size))
        self.assertEqual(["1mn 1s", "1mn 1s 394ms", "1mn 1s", "00:01:01.394"], general_track.other_duration)
    ...
    def test_raises_on_nonexistent_library(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with pytest.raises(OSError) as exc:
                MediaInfo.parse(os.path.join(data_dir, "sample.mp4"), library_file=nonexistent_library)
...
@pytest.mark.skipif(
    sys.version_info < (3, 7) or sys.platform == "win32",
...
class MediaInfoCoverDataTest(unittest.TestCase):
    def setUp(self):
        self.cover_mi = MediaInfo.parse(os.path.join(data_dir, "sample_with_cover.mp3"), cover_data=True)
...
class MediaInfoSlowParseTest(unittest.TestCase):
    def setUp(self):
        self.media_info = MediaInfo.parse(os.path.join(data_dir, "vbr_requires_parsespeed_1.mp4"), parse_speed=1)
...
class MediaInfoOptionsTest(unittest.TestCase):
    def setUp(self):
        ...
        if lib_version < (19, 9):
            pytest.skip(
                "The Reset option is not supported by this library version " "(v{} detected, v19.09 required)".format(
                    lib_version_str
                )
            )
...
class MediaInfoOutputTest(unittest.TestCase):
    ...
    def test_json_output(self):
        ...
        if lib_version < (18, 3):
            pytest.skip(
                "This version of the library does not support JSON output " "(v{} detected, v18.03 required)".format(
                    lib_version_str
                )
            )
    ...
    def test_parameter_output(self):
        media_info = MediaInfo.parse(os.path.join(data_dir, "sample.mp4"), output="General;%FileSize%")
